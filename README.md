...
用户查询: "红色杯子"  (文本)
 ...
 URDF      SDXL-Turbo        image → 4K PBR GLB mesh      │
   │         ┌────────────────────────────────────────────┘
   ▼         ▼
...
robotsmith generate "red ceramic mug" --image reference.png                     # 默认 TRELLIS.2
...
|------|------|:---:|------|------|------|
| **`trellis2`** | [TRELLIS.2-4B](https://github.com/ZJLi2013/TRELLIS.2/tree/rocm) | ✅ | ≥24 GB | ✅ MI308X | **默认** — 4K PBR, 无底座 artifact, 无 bpy 依赖 |
...
```
参考图 → TRELLIS.2-4B (4B, ~275s, ≥24GB) → O-Voxel → remesh + 4K PBR bake → GLB
```

> TRELLIS.2 优势：4K PBR 纹理完整嵌入 GLB（38 MB），无底座 artifact，无 bpy 依赖，mesh 质量显著优于 Hunyuan3D。
...
> 详细对比见 [docs/design.md](docs/design.md#trellis2-vs-hunyuan3d-e2e-对比red-ceramic-mug-mi308x-512³)。
...
- [TRELLIS.2](https://github.com/ZJLi2013/TRELLIS.2/tree/rocm) — **默认 3D 后端** (ROCm fork, 4K PBR)
