...
| 轮次 | 修改 | 结果 |
|------|------|------|
| r0 (baseline) | 无 | 物体掉落到地面（workspace_xy 超出 table URDF 范围）；相机被 table 遮挡 |
| r1 | genesis_loader: table pos=(workspace_center_x, 0, 0)；camera 改为 2 视角 | 物体全部在桌面上（mug、bowl、3 blocks）；Franka 不在画面中 |
| r2 | camera lookat 调整为 (cx*0.5, cy, table_z)，FOV=55 | **全部物体在桌面，Franka 可见** |

**r2 观察：**
- overview: 能看到 table 全貌 + 5 个物体 + Franka 手臂（在画面左下，home pose）
- topdown: Franka 部分可见（左侧），table 边界清晰，物体全部在桌面内
- bowl 靠近桌面边缘但仍在范围内（y=-0.189, 桌面 y 范围 ±0.3）
- 无纹理问题不影响 layout 验证（已知限制：Docker image Genesis 0.4.3 不完全支持 GLB PBR）

**剩余小问题：**
- bowl 看起来像漂浮（stable_pose.z 可能过高），但 settle 后稳定在桌面上
- overview 仍未完整显示 Franka base，但够用

### 结论与 Next Step

**结论**：table position offset 修复解决了核心 layout 问题。当前 `tabletop_simple` 场景 layout 可用。

**Next Step（按优先级）**：
1. 用 `--scene tabletop_simple` 跑 `collect_data.py` 端到端验证（先 smoke test 2 episodes）
2. 后续 P1: 升级 Docker image 到 Genesis 0.4.5 + ROCm 7.1 以支持 PBR 纹理渲染
