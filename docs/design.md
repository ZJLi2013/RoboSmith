...
│                    │
│                    └→ 未命中 → T2I (SDXL-Turbo) → Hunyuan3D (Shape+PBR) → mesh_cleanup → URDF → 入库
│
...
用户请求: "红色水杯" / 图片 / 文本描述
  ...
  (URDF)      │ T2I 桥接      │  text → 参考图 (SDXL-Turbo, 3D 友好 prompt)，有图则跳过
              └──────┬───────┘
                     ▼
              ┌──────────────┐
              │ 3D Shape     │  image → 白模 mesh (Hunyuan3D-Shape, 3.3B, ~60s)
              └──────┬───────┘
                     ▼
              ┌──────────────┐
              │ PBR Paint    │  白模 → 带纹理 GLB (Hunyuan3D-Paint, 2B, ~30-60s)
              └──────┬───────┘
                     ▼
              ┌──────────────┐
...
|------|------|:---:|:---:|
| **Objaverse 策划** ← 默认 | 高频类目（10 种桌面物品） | ★★★★☆ 人工建模/3D 扫描 | **无** |
| **Hunyuan3D 生成** ← 兜底 | 搜索未命中的长尾类目 | ★★★☆☆ 生成模型 | 有（需 trim） |
| 程序化原语 ← 已废弃 | 仅供 table / ground plane | ★☆☆☆☆ 几何体 | 无 |

> 详见 §1.6.2 社区方案对比。高频类目用策划资产避免生成 artifact，生成 pipeline 作为兜底。
...
### 当前方案：Hunyuan3D-2.1（默认后端）

| 项目 | 值 |
|------|-----|
| 模型 | [Hunyuan3D-2.1](https://github.com/Tencent-Hunyuan/Hunyuan3D-2.1) (Tencent, 3.3B shape + 2B PBR) |
| 输入 | **单张参考图片** (image-to-3D) |
| 输出 | **GLB (带 PBR 纹理)**，默认 visual.glb + collision.obj |
| 速度 | ~60s (shape) + ~30-60s (paint) = **~90-120s 总计** |
| VRAM | **~29 GB**（shape 10 GB + paint 21 GB 同时加载） |
| ROCm | **✅ 已验证** — MI300X, ROCm 6.4, AOTriton FA, 零修改 |
| 模式 | **默认: shape + PBR paint**；可选 `texture=False` 仅 shape（白模，10 GB，60s） |
...
| 步骤 | 工具 | 说明 |
|------|------|------|
| **底座清理** | `mesh_cleanup.py` | 移除底部平面 artifact（法向朝下 + 底部 5% 高度），详见 §1.6.1 |
...
> 结论：生成模型的底座 artifact 是当前技术的固有限制。工业级项目（GraspVLA, InternVLA-A1）
> 选择绕开生成模型，直接使用策划资产。RoboSmith 的定位是"搜索不到再生成"，
> 因此两种路径都需要 — 扩大策划资产库（减少生成频率）+ 改善生成后处理（提升生成质量）。
...
L2      材质精确的物理属性                  ❌ 尚未实现
L3      视觉逼真（PBR、纹理、颜色）         ✅ Hunyuan3D PBR Paint
```
...
| 纹理 | ✅ PBR Paint 已启用 | — | ✅ 已解决 |
| 网格质量 | ✅ Hunyuan3D 344K verts | — | ✅ 已解决 |
...
| 优先级 | 任务 | 方案 |
|--------|------|------|
| P0 | 底座 artifact 去除 | Mesh trim（§1.6.1） |
