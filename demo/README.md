...
- T2I 桥接已验证：SDXL-Turbo 在 MI300X + ROCm 6.4 上 0.3s/img，8GB VRAM

## 前提条件

| 项目 | 要求 |
|------|------|
| GPU | AMD MI300X / MI300X (ROCm 6.4+) 或 NVIDIA (CUDA 12+) |
...
两种模式均已在 MI300X + ROCm 6.4 上验证通过（RS-6、RS-9）。

## 实验结果展示

### RS-6: T2I 桥接 — 3D 重建友好生成效果

以下图片在 MI300X + ROCm 上自动生成，作为 Hunyuan3D-2.1 的输入参考图。
...
> Base 的低 CFG (4.5) 修复了复杂背景但牺牲了颜色精度。
...
`glass` 等关键词。注意 CLIP 有 77 token 限制。
